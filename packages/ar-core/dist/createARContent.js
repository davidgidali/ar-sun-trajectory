 ...
 */
export function createARContent(scene, trajectory, THREE) {
    const aboveHorizonPoints = [];
    const belowHorizonPoints = [];
    ...
    const createCompassArrow = (direction, position, color, label) => {
        // Create arrow pointing in direction, lying flat on ground
        const arrow = new THREE.ArrowHelper(direction, new THREE.Vector3(position.x, -0.1, position.z), 2, // length
        color, 0.6, // head length
        0.4 // head width
        );
    ...
    if (trajectory.currentPosition) {
        ...
        const currentPoint = new THREE.Vector3(currentPos3D.x * 10, currentPos3D.y * 10, currentPos3D.z * 10);
